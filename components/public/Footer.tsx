    ...
    <footer className="bg-[#060f22] text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-playfair font-bold text-white">K.L. International</p>
                <p className="text-orange-300 text-xs">School</p>
              </div>
            </div>
            <p className="text-white/60 text-sm leading-relaxed mb-4">
              Nurturing young minds with excellence in education, values, and holistic development since establishment.
            </p>
            <a
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[['/', 'Home'], ['/about', 'About Us'], ['/academics', 'Academics'], ['/admissions', 'Admissions'], ['/gallery', 'Gallery'], ['/contact', 'Contact']].map(([href, label]) => (
                <li key={href}>
          ...
          {/* Programs */}
          <div>
            <h3 className="font-semibold text-white mb-4 text-sm uppercase tracking-wider text-[#FF7A00]">Programs</h3>
            <ul className="space-y-2">
              {['Nursery & Pre-Primary', 'Primary (Class 1-5)', 'Middle School (6-8)', 'Secondary (9-10)', 'Senior Secondary (11-12)'].map((p) => (
                <li key={p}>
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                ...
                <span className="text-white/60 text-sm">Hastinapur Road, Gwalior (MP)</span>
              </li>
              <li>
              ...
              <li>
                <a href="mailto:support@klinternational.space" className="flex items-center gap-3 text-white/60 hover:text-white text-sm transition-colors">
                  <Mail className="w-4 h-4 text-[#FF7A00] shrink-0" />
                  support@klinternational.space
                </a>
              </li>
            </ul>
          </div>
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-white/40 text-xs">© {new Date().getFullYear()} K.L. International School. All rights reserved.</p>
          <p className="text-white/40 text-xs">Hastinapur Road, Gwalior (MP)</p>
        </div>
      </div>
    </footer>
  );
